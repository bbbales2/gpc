...
deg = 4
xh, wh = scipy.special.la_roots(deg, alpha)#numpy.polynomial.laguerre.laggauss(deg)
...
for k in range(K):
    ...
    uks.append(u / (math.gamma(k + alpha + 1) / math.factorial(k))**2)
...
def approx(z):
    ...
    for k in range(K):
        ...
        s = scipy.special.eval_genlaguerre(k, alpha, z)
        #s = numpy.polynomial.laguerre.lagval(z, c)
...
labels = []
for z in numpy.linspace(0.1, 1.0, 10):#[1.0, 1.5]:
    plt.plot(approx(z))
...
deg = 10
xh, wh = scipy.special.la_roots(deg, alpha)
for x, w in zip(xh, wh):
    denominator += w * L(x) * scipy.stats.norm.pdf(x)

    #plt.plot(solve(D(x))[0], 'b')
    #plt.plot(approx(x), 'r')
    #plt.show()
...
plt.plot(D(zs), post / max(post), 'r')
plt.plot(D(zs), scipy.stats.norm.pdf(zs) / max(scipy.stats.norm.pdf(zs)), 'b')
